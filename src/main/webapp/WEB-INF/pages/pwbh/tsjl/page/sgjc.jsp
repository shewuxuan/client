        ...
        <form class="layui-form layui-form-pane" action="">
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <textarea id="sgjcbeizhu" placeholder="" class="layui-textarea"></textarea>
                </div>
            </div>
        </form>
    </blockquote>
    ...
    layui.use('table', function () {
        ...
        let tableReload = table.render({
            ...
            , cols: [[ //表头
                ...
                , {field: 'nr', title: '检查内容', width: '77%', align: 'center', sort: true
                    , templet: function (d) {
            ...
            , done: function (res) {
                ...
                autosave = setInterval(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${basePath}/pwbh_jl_sgjc/updateBatch",              // 改
                        data: JSON.stringify(record),//必须
                        contentType: "application/json;charsetset=UTF-8",//必须
                        dataType: "json",//必须
                        success: function (data) {
                            if (data.code !== 0) {
                                console.log("sgjc automatically saved failed!");
                            }
                        }
                    });
                }, 10000);
        ...
        // 编辑
        table.on('edit(sgjc)', function (obj) {         // 改
            let data = obj.data;
        });
        ...
        //监听事件
        table.on('toolbar(sgjc)', function (obj) {      // 改
            switch (obj.event) {
                // 添加
                case 'ADD':
                    ...
                    if (submitJl(record)) {
                        layer.open({
                            ...
                            area: ['800px', '520px'],
                            content: '${basePath}/pwbh/tsjl/add/addSgjc/' + tsid + '/' + ssqy
                            , end: function () {
                                clearTimeout(autosave);
                                tableReload.reload();
                            }
        ...
        //监听行操作
        table.on('tool(sgjc)', function (obj) {
            let that = obj.data;
            if (obj.event === 'del') {
                layer.confirm('真的删除吗？', function () {
                    ...
                    if (submitJl(record)) {
                        // 删除
                        $.get("${basePath}/pwbh_jl_sgjc/deleteByPrimaryKey/" + that.id, function (result) {
                            clearTimeout(autosave);
                            tableReload.reload();
...
<script type="text/html" id="sgjcjg">
    <select name='select4sgjc' lay-ignore lay-filter="sgjc" lay-search=''>
