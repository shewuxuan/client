...
<body>
    <table id="dzjc" lay-filter="dzjc"></table>
</div>
...
<script>
    layui.use('table', function () {
                ...
                autosave = setInterval(function () {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.code !== 0) {
                                console.log("dzjc automatically saved failed!");
                            }
                        }
                    });
                }, 10000);
                record = resdata;
                $("[name='select4dzjc']").change(function () {
        ...
        // 编辑
        table.on('edit(dzjc)', function (obj) {         // 改
            let data = obj.data;
        ...
        //监听事件
        table.on('toolbar(dzjc)', function (obj) {      // 改
            switch (obj.event) {
                // 添加
                case 'ADD':
        ...
        //监听行操作
        table.on('tool(dzjc)', function (obj) {
            let that = obj.data;
            if (obj.event === 'del') {
...
<%-- 结果状态列(正常/异常)--%>
<script type="text/html" id="dzjcjg">
    <select name='select4dzjc' lay-ignore lay-filter="dzjc" lay-search=''>
