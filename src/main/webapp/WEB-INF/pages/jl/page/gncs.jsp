    ...
    layui.use('table', function () {
        ...
        let tableReload = table.render({
            ...
            , cols: [[ //表头
                ...
                , {
                    field: 'jsbz', title: '技术标准', width: '55%', align: 'center', sort: true, templet: function (d) {
            ...
            , done: function (res, curr, count) {
                ...
                autosave = setInterval(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${basePath}/jlgncs/updateBatch",              // 改
                ...
                for (let i = 0, length = resdata.length; i < length; i++) {
                    if (
                        resdata[i].jg == null || resdata[i].jg === "" || resdata[i].jg == -1
                    ) {
                ...
                case 'ADD':
                    ...
                    if (submitJl(record)) {
                        layer.open({
                            ...
                            area: ['800px', '520px'],
                            content: '${basePath}/jl/add/addGncs/' + tsid + '/' + ssqy
                            , end: function () {
                                clearTimeout(autosave);
                                tableReload.reload();
                            }
        ...
        table.on('tool(gncs)', function (obj) {
            ...
            if (obj.event === 'del') {
                layer.confirm('真的删除吗？', function () {
                    ...
                    if (submitJl(record)) {
                        // 删除
                        $.get("${basePath}/jlgncs/deleteByPrimaryKey/" + that.id, function (result) {
                            clearTimeout(autosave);
                            tableReload.reload();
...
<script type="text/html" id="gncsjg">
    <select name='select4gncs' lay-ignore lay-filter="gncs" lay-search=''>
