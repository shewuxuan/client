        ...
        <form class="layui-form layui-form-pane" action="">
            <div class="layui-form-item layui-form-text">
                ...
                <div class="layui-input-block">
                    <textarea id="dyxnbeizhu" placeholder="" class="layui-textarea"></textarea>
                </div>
            </div>
        </form>
    </blockquote>
    ...
    layui.use('table', function () {
        ...
        let tableReload = table.render({
            ...
            , cols: [[ //表头
                ...
                , {
                    field: 'jsbz', title: '技术标准', align: 'left', width: '48%', sort: true, templet: function (d) {
                        return '<div style="text-align: left">' + d.jsbz + '</div>';
                    }
                }
                , {field: 'jcjg', title: '检查结果', align: 'center', width: '10%', templet: '#dyxnjg'}
                , {title: '操作', align: 'center', width: '8%', toolbar: '#barDemo'}
            ]]
            , done: function (res) {
                ...
                autosave = setInterval(function () {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "${basePath}/jldyxn/updateBatch",              // 改
                        data: JSON.stringify(record),//必须
                        contentType: "application/json;charsetset=UTF-8",//必须
                        dataType: "json",//必须
                        success: function (data) {
                            if (data.code !== 0) {
                                console.log("dyxn automatically saved failed!");
                            }
                        }
                    });
                }, 10000);
                record = resdata;
                // 设置导航条背景色
                $("[name='select4dyxn']").change(function () {
                ...
                for (let i = 0, length = resdata.length; i < length; i++) {
                    if (
                        ...
                        $("#li_dyxn").css({"background-color": ""});
                        return;
                    }
                    $("#li_dyxn").css({"background-color": "#009688"});
                }
            }
        });

        // 编辑
        table.on('edit(dyxn)', function (obj) {         // 改
            let data = obj.data;
        });
...
//监听事件
        table.on('toolbar(dyxn)', function (obj) {      // 改
            switch (obj.event) {
                // 添加
                case 'ADD':
                    ...
                    if (submitJl(record)) {
                        layer.open({
                            ...
                            area: ['800px', '520px'],
                            content: '${basePath}/jl/add/addDyxn/' + tsid + '/' + ssqy
                            , end: function () {
                                clearTimeout(autosave);
                                tableReload.reload();
                            }
        ...
        //监听行操作
        table.on('tool(dyxn)', function (obj) {
            let that = obj.data;
            if (obj.event === 'del') {
                layer.confirm('真的删除吗？', function () {
                    ...
                    if (submitJl(record)) {
                        // 删除
                        $.get("${basePath}/jldyxn/deleteByPrimaryKey/" + that.id, function (result) {
                            clearTimeout(autosave);
                            tableReload.reload();
...
<script type="text/html" id="dyxnjg">
    <select id="" name='select4dyxn' lay-ignore lay-filter="dyxnjg" lay-search=''>
