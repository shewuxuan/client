...
import com.guodu.service.impl.dtu.JlCyjdServiceImpl;
...
public class JlCyjdController {
    ...
    public String insert(@RequestBody JlCyjd record, HttpServletRequest request) {
        ...
        try {
            ...
            int i = jlCyjdServiceImpl.insert2(record);
            map.put("code", 0);
            map.put("msg", "添加成功！");
            map.put("data", i);
    ...
    public String batchDeleteByPrimaryKey(@RequestBody List<String> ids) {
        ...
        try {
            ...
            map.put("msg", "删除成功");
            map.put("data", ids);
            StaticLog.info("jl_cyjd表【删除】成功 -----> " + ids, "INFO");
        } catch (Exception e) {
            map.put("code", -1);
            map.put("msg", "删除失败！");
            StaticLog.error(e, "jl_cyjd表【删除】失败 -----> " + ids, "ERROR");
        }
        return JSONUtil.toJsonStr(map);
    }
}
